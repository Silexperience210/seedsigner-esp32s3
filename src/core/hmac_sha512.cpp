...
// SHA512 implementation needed for HMAC-SHA512
...
static void sha512(const uint8_t* data, size_t len, uint8_t hash[64]) {
    uint64_t state[8] = {
        0x6a09e667f3bcc908ULL, 0xbb67ae8584caa73bULL, 0x3c6ef372fe94f82bULL, 0xa54ff53a5f1d36f1ULL,
        0x510e527fade682d1ULL, 0x9b05688c2b3e6c1fULL, 0x1f83d9abfb41bd6bULL, 0x5be0cd19137e2179ULL
    };

    size_t bit_len_hi = (len >> 61);
    size_t bit_len_lo = len << 3;

    // Padding
    size_t pad_len = (len % 128 < 112) ? (112 - len % 128) : (240 - len % 128);
    size_t total_len = len + pad_len + 16;
    ...
    for (int i = 0; i < 8; i++) {
        padded[total_len - 16 + i] = (bit_len_hi >> (56 - i * 8)) & 0xFF;
        padded[total_len - 8 + i] = (bit_len_lo >> (56 - i * 8)) & 0xFF;
    }

    for (size_t i = 0; i < total_len; i += 128) {
        sha512_transform(state, padded + i);
    }
...
void hmac_sha512(const uint8_t* key, size_t key_len, 
    ...
    uint8_t* inner = new uint8_t[128 + data_len];
    memcpy(inner, ipad, 128);
    memcpy(inner + 128, data, data_len);
    sha512(inner, 128 + data_len, inner_hash);
    delete[] inner;
    
    // Outer hash: SHA512(opad || inner_hash)
    uint8_t* outer = new uint8_t[128 + 64];
    memcpy(outer, opad, 128);
